...
start_x_k = 0.7
start_y_k = 0.7  # The algorithm starts at (x,y)=(0.7,0.7)
...
def df(x, y): return np.array([(6-12*x**2)*y*math.exp(-x ** 2-y ** 2),
                               (6-12*y**2)*x*math.exp(-x ** 2-y ** 2)])  # Gradient of our function
...
def momentum_gd():
    ...
    while previous_step_size > precision and iters < max_iters:
        ...
        prev_v_k = v_k
        # calculates momentum
        v_k = mom_term*prev_v_k + rate*stochastic_gradiend * df(prev_x_k, prev_y_k)
        cur_x_k, cur_y_k = np.array(
...
x = np.linspace(-4, 2.5, 500)
y = np.linspace(-4, 2, 500)
...
ax.text(-3.9, -3.9, 'Nº iteraciones GD:' + str(len(gr_gd))+'\n Nº iteraciones SGD:' + str(len(gr_sgd1))+'\nNº iteraciones Adam: ' +
